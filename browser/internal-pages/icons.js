/**
 * THAAW Browser — Client-Side SVG Icon Helper for Internal Pages
...
window.SVG_ICONS = {
  ...
  // Shortcuts & Search Engine Brand SVGs
...
window.getSvgIcon = function(name, size = 18, className = '') {
  const content = window.SVG_ICONS[name] || window.SVG_ICONS['shield'];
...
// Automatically populate elements with data-icon attribute
document.addEventListener('DOMContentLoaded', () => {
  document.querySelectorAll('[data-icon]').forEach(el => {
    const iconName = el.getAttribute('data-icon');
    const iconSize = parseInt(el.getAttribute('data-icon-size') || '18', 10);
    if (iconName && window.SVG_ICONS[iconName]) {
      el.innerHTML = window.getSvgIcon(iconName, iconSize);
    }
  });
});
